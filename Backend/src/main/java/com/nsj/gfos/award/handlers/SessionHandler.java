...
/**
 * Die Klasse <i>SessionHandler</i> stellt Hilfsmethoden bereit, welche sich um
 * das Management der Sessions und anderer Funktionen bei An- und Abmeldung
 * kümmern.
 * 
 * @author Schnuels
 */
public class SessionHandler {
	...
	/**
	 * Die Methode <i>createSession</i> erstellt mit Hilfe einer SessionID und einer
	 * Personalnummer eine neue Session auf der Datenbank und gibt nach erfogreichem
	 * Prozess eine Erfolgsmeldung zurück, in welcher das Objekt des angemeldeten
	 * Mitarbeiters enthalten ist.
	 * 
	 * @param args - Ein Array mit der SessionID und der Personalnummer als Inhalt
	 * @return String - Eine Fehlermeldung bei einem Fehler und eine Erfolgsmeldung
	 *         mit Mitarbeiter Objekt bei einem Erfolg
	 */
	public static String createSession(String[] args) {
		...
		try {
			...
			try {
				...
				if (!rSet.next())
					return JsonHandler.fehler("Fehler bei Zugriff auf die Datenbank");
				Mitarbeiter obj = QueryHandler.createMitarbeiterFromQuery(rSet,
						new String[] { "Personalnummer", "Name", "Vorname", "erreichbar", "Arbeitskonto", "EMail",
								"Passwort", "Status", "Rechteklasse", "Abteilung", "Vertreter", "gda" });
	...
	/**
	 * Die Methode <i>closeSession</i> schließt eine vorhandene Session mit Hilfe
	 * der SessionID.
	 * 
	 * @param sessionID - ID der zu schließenden Session
	 * @return String - Je nach Ausgang des Vorgangs eine Fehler- oder
	 *         Erfolgsmeldung
	 */
	...
	/**
	 * DIe Methode <i>checkSessionID</i> prüft, ob zu einer gegebenen SessionID eine
	 * aktive Session auf der Datenbank hinterlegt ist.
	 * 
	 * @param sessionID - zu prüfende SessionID
	 * @return boolean - true, wenn eine Session für die ID existiert, false, wenn
	 *         nicht
	 */
	...
	/**
	 * Die Methode <i>changeStatus</i> ändert den Status eines Mitarbeiters auf der
	 * Datenbank.
	 * 
	 * @param pn     - Personalnummer des Mitarbeiters
	 * @param status - neu einzutragender Status
	 * @return boolean - true, wenn der Status geändert werden konnte, false, wenn
	 *         nicht
	 */
	...
	/**
	 * Die Methode <i>changeErreichbar</i> ändert den Wert für 'erreichbar' eines
	 * Mitarbeiters auf der Datenbank.
	 * 
	 * @param pn     - Personalnummer des Mitarbeiters
	 * @param status - neu einzutragender Wert für 'erreichbar'
	 * @return boolean - true, wenn 'erreichbar' geändert werden konnte, false, wenn
	 *         nicht
	 */
	...
	/**
	 * Die Methode <i>checkIfUSerIsConnected</i> prüft anhand der gegebenen
	 * Personalnummer, ob eine aktive Session dieses Mitarbeiters existiert.
	 * 
	 * @param pn - Personalnummer des Mitarbeiters
